...
class PersonSpeedComparison:
    ...
    async def toEmbed(self, requesterID = None):
        ...
        embed.add_field(name=f"**{self.viewer.nickname}** Speeds", value=(
            f"{emojis.blank}{emojis.blank} **Walk Speed:** {self.viewer.walkperhour:,.3mu} per hour\n"
            f"{emojis.blank}{emojis.blank} **Run Speed:** {self.viewer.runperhour:,.3mu} per hour"), inline=False)
