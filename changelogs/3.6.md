...
- Added `&toggleincomprehensible`, which makes the users size indeterminate, and has funny affects in stats, comparisons, and sizetags.
...
- `&stat lift` would fail, even though `lift` is a valid stat.
